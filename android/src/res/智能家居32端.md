...
- 闹钟
```
   cJSON* autoModeData = cJSON_GetObjectItem(smart_set, "alarmController");
    if (alarmClock == NULL) {
        printf("Error: alarmController not found in JSON\n");
        cJSON_Delete(root);
        return NULL;
    }else{
    autoControllerBean->name = strdup(cJSON_GetObjectItem(alarmClock, "name")->valuestring);
    autoControllerBean->low = cJSON_GetObjectItem(alarmClock, "low")->valueint;
    autoControllerBean->high = cJSON_GetObjectItem(alarmClock, "high")->valueint;
    autoControllerBean->stateHigh = cJSON_GetObjectItem(alarmClock, "stateHigh")->valueint;
    autoControllerBean->stateLow = cJSON_GetObjectItem(alarmClock, "stateLow")->valueint;
