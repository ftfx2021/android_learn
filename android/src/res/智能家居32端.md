...
var key = ""
var value = 0
var syncJsonStr = "{\"smart_set\":{\"${key}\":${value}}}"
key = "bedroomLed";
value = aplicationCahce.bedroomLed;
sendingDataWithDialog(syncJsonStr,"正在同步，已同步0/7){it->
  if(it){
  syncFlag = true
  }
  else{syncFlag=false}
...
if(syncFlag){
  key = "livingLed";
  value = aplicationCahce.livingLed;
  sendingDataWithDialog(syncJsonStr,"正在同步，已同步1/7){it->
  if(it){
  syncFlag = true
  }
  else{syncFlag=false}
