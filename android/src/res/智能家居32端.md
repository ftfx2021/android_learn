...
(5) 安卓上线：
        ...
        Android每60s发送一次握手请求：安卓发送用户信息，响应时间2s.要求控制数据和握手请求数据不能同时发送，用一个sendingDataViewModel限制:while(sendingDataViewModel.value==false)
        若收到ack-》设备存活
        STM32若超过60s没收到安卓的信息，则用户离线。实现：time，每收到用户信息时time就归0   ，发送ack     
(6)STM32上线
      发送ack
  (7) 初始化逻辑修改：初始化esp8266->初始化阿里云，某一步骤出错，直接跳出，给出出错信息。仅当不出错时才进行联网操作
      esp8266初始化出错，仅提示，不做处理；阿里云初始化出错，每隔30s重连，使用全局变量,u8 isNeedReconnect;
  (8)发送数据逻辑修改：数据下发时，3s内才可再次下发 isEnable = sendingDataViewModel.value;
```
 private lateinit var sendDeviceStateJob: Job
class CommonViewModel : ViewModel() {
    var handshakeTimes = mutableStateOf(0);
    var isDeviceOnline = mutableStateOf(false);
}
private fun startSendDeviceStateJob(viewModel:CommonViewModel) {
    sendDeviceStateJob = CoroutineScope(Dispatchers.Default).launch {
        while (isActive) {
            delay(60000)
            sendAck()
              withContext(Dispatchers.Main) {//主线程更新viewModel
                viewModel.handshakeTimes.value++
                if (viewModel.handshakeTimes.value > 3) {
                    viewModel.isDeviceOnline.value = false
                }
            }
        }
    }
  ...
  其它发送事件
    {
// 在需要重新启动定时任务的地方，再次调用这个函数
{
    sendDeviceStateJob.cancel()
    // 执行其他发送事件
    // ...
    startSendDeviceStateJob()
}
}
```
```
...
onMsgArriver(){
//有数据过来：说明设备存活：修改isDeviceOnline.value = true，handshakeTimes.value = 0;
}
```
```
class CommonViewModel : ViewModel() {
    var switchEnable = mutableStateOf(true)
}
@Composable
fun SwitchWithDelay(
   ...
   DisposableEffect(key1 = viewModel.switchEnable.value) {
        val coroutineScope = CoroutineScope(Dispatchers.Main)
        val job = coroutineScope.launch {
            if (!viewModel.switchEnable.value) {
                withContext(Dispatchers.Default) {
                    delay(3000)
                }
                viewModel.switchEnable.value = true
            }
        }
    ...
    Switch(
        ...
        onCheckedChange = { newCheckedState ->
            onCheckedChange(newCheckedState)
            viewModel.switchEnable.value = false
        },
        enabled = viewModel.switchEnable.value,
        colors = colors,
