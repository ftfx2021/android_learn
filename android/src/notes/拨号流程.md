...
```
- 进入其实现类**RIL.java,RILJ封装了一个RILRequest并调用HIDL接口开始向RILC发送消息，最终到达了modem并和它进行通信**
> RILJ对象在接收到Telephony Frameworks发出的RIL请求后，将这些请求封装成RILRequest对象，它会保存RILJ发出Solicited请求消息类型以及Callback回调使用的Message消息对象。
```
 @Override
    public void dial() {
